define([ 'jquery', 'underscore', 'twigjs', 'lib/components/base/modal' ], function ($, _, Twig, Modal) {
  let CustomWidget = function () {
    ...
    this.renderDistBlock = () => {


      self.getTemplate(  // render usersListPercent

        'distBlock', // a name our template

        {}, // i don't know why // FIXME

        ( template ) => { // our callback

          self.distBlock = template.render( {
            usersList: self.usersList
          } );

          // console.debug( self.distBlock );

          self.render_template(
            {
              caption: {
                class_name: "js-ac-caption leadsDist",
                html: "",
              },
              body: "",
              render: self.distBlock
            }
          );
    ...
    this.renderUsersListPercent = () => {

      self.getTemplate(  // render usersListPercent

        'usersListPercent', // a name our template

        {}, // i don't know why // FIXME

        ( template ) => { // our callback

          self.usersListPercent = template.render( {
            users: AMOCRM.constant( "managers" ),
            serverAddress: self.serverAddress
          } );

          // console.debug( self.usersListPercent );

          self.renderDistBlock();
        }
      );

    }

    this.renderUsersListEven = () => {

      self.getTemplate(  // render usersListEven

        'usersListEven', // a name our template

        {}, // i don't know why // FIXME

        ( template ) => { // our callback

          self.usersList = template.render( { users: AMOCRM.constant( "managers" ) } );

          // console.debug( self.usersList );

          self.renderUsersListPercent();
        }
      );

    }

    this.renderDistPanel = () => {
      self.renderUsersListEven();
    }
    ...
    this.callbacks = {
      ...
      init: function () {
        ...
        if ( !$( 'link[href="' + self.get_settings().path + '/style.css?v=' + self.get_settings().version +'"' ).length )
        {
          //  Подключаем файл style.css передавая в качестве параметра версию виджета
          $( "head" ).append( '<link type="text/css" rel="stylesheet" href="' + self.get_settings().path + '/style.css?v=' + self.get_settings().version + '">' );
        }
